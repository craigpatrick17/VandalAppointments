...
NSString * comvandalappointmentsprototype$ModuleRequireFormat = @"(function(exports){"
...
@implementation comvandalappointmentsprototypeObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_comvandalappointmentsprototype);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_comvandalappointmentsprototype gc];
}
...
{
	// create comvandalappointmentsprototype global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "comvandalappointmentsprototype" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_comvandalappointmentsprototype = [[comvandalappointmentsprototypeObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_comvandalappointmentsprototype];
	
	NSString *_comvandalappointmentsprototypeNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _comvandalappointmentsprototypeNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_comvandalappointmentsprototype valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:comvandalappointmentsprototype$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.comvandalappointmentsprototype.kroll" 
