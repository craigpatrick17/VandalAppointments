...
//Rare exceptions to indicate a bug in the _comvandalappointmentsprototype code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_comvandalappointmentsprototype = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_comvandalappointmentsprototype]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.comvandalappointmentsprototype.module"
